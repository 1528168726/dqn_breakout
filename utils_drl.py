...
class Agent(object):
    ...
    def __init__(
            ...
            gamma: float,
            seed: int,
            remove_param: float,             # added by myself

            eps_start: float,
            eps_final: float,
            eps_decay: float,

            restore: Optional[str] = None,      # file path of pre-trained weights
    ) -> None:
        self.__action_dim = action_dim
        self.__device = device
        self.__gamma = gamma
        self.__removed_param = remove_param
        ...
        if restore is None:
            self.__policy.apply(DQN.init_weights)       # 对__policy的每一层参数都使用init_weights()
        else:
            self.__policy.load_state_dict(torch.load(restore))
        self.__target.load_state_dict(self.__policy.state_dict()) # 将__policy的每一层参数加载到__target中
    ...
    def learn(self, memory: ReplayMemory, batch_size: int) -> float:
        ...
        values_next = self.__target(next_batch.float()).max(1).values.detach() # 返回相同的Tensor，但不会算入反向传播中
        moved = (action_batch == 0).to(torch.int32)
        expected = (self.__gamma * values_next.unsqueeze(1)) * \
            (1. - done_batch) + reward_batch + moved * self.__removed_param
